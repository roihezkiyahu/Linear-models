...
we can see that at least one of the covariates is significant according to the pvalue,lets the interaction check it with a nested model:
...
we can see that the interaction is significant therefore the lines are not parallel, but the low R squered suggets that the model does not fir the data.
...
our pvalue is lower and out R squared is higher which suggestes that the model better fits the data, lets see it also visually
